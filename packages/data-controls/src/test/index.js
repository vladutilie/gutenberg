...
describe( 'controls', () => {
	...
	describe( 'SELECT', () => {
		...
		const registryMock = {
			...
			subscribe: jest.fn( ( subscribeCallback ) => {
		...
		it( 'invokes selector with undefined resolver', () => {
			...
			const value = testControl( getSelectorArgs( 'mockStore', 'selectorWithUndefinedResolver' ) );
		...
		it( 'invokes selector with resolver set to false', () => {
			const testControl = controls.SELECT( registryMock );
			const value = testControl( getSelectorArgs( 'mockStore', 'selectorWithFalseResolver' ) );
		...
		describe( 'invokes selector with resolver set to true', () => {
			...
			it( 'returns a promise', () => {
				const value = testControl( getSelectorArgs( 'mockStore', 'selectorWithResolver' ) );
				expect( value ).toBeInstanceOf( Promise );
				expect( hasFinishedResolution ).toHaveBeenCalled();
				expect( registryMock.subscribe ).toHaveBeenCalled();
			} );
			it( 'selector with resolver resolves to expected result when ' + 'finished', async () => {
				const value = testControl( getSelectorArgs( 'mockStore', 'selectorWithResolver' ) );
