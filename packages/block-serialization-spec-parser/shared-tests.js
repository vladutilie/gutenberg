export const jsTester = ( parse ) => () => {
	describe( 'output structure', () => {
		test( 'output is an array', () => {
			...
			expect( parse( '<!-- wp:block --><!-- wp:inner /--><!-- /wp:block -->' ) ).toEqual(
				expect.any( Array )
			);
		...
		test( 'parses blocks of various types', () => {
			...
			expect( parse( '<!-- wp:void {} /-->' )[ 0 ] ).toHaveProperty( 'blockName', 'core/void' );
			expect( parse( '<!-- wp:void {"value":true} /-->' )[ 0 ] ).toHaveProperty(
				'blockName',
				'core/void'
			);
			...
			expect( parse( '<!-- wp:block --><!-- /wp:block -->' )[ 0 ] ).toHaveProperty(
				'blockName',
				'core/block'
			);
			expect( parse( '<!-- wp:block {} --><!-- /wp:block -->' )[ 0 ] ).toHaveProperty(
			...
			expect( parse( '<!-- wp:block {"value":true} --><!-- /wp:block -->' )[ 0 ] ).toHaveProperty(
				'blockName',
				'core/block'
			);
			expect( parse( '<!-- wp:block {} -->inner<!-- /wp:block -->' )[ 0 ] ).toHaveProperty(
				'blockName',
				'core/block'
			);
			expect(
				parse( '<!-- wp:block {"value":{"a" : "true"}} -->inner<!-- /wp:block -->' )[ 0 ]
		...
		test( 'JSON attributes are key/value object', () => {
			...
			expect( parse( '<!-- wp:void {} /-->' )[ 0 ] ).toHaveProperty( 'blockName', 'core/void' );
			expect( parse( '<!-- wp:void {} /-->' )[ 0 ] ).toHaveProperty( 'attrs', {} );
			expect( parse( '<!-- wp:void {"key": "value"} /-->' )[ 0 ] ).toHaveProperty( 'attrs', {
				key: 'value',
			} );
			...
			expect( parse( '<!-- wp:block {"key": "value"} --><!-- /wp:block -->' )[ 0 ] ).toHaveProperty(
				'attrs',
				{ key: 'value' }
			);
		} );
		...
		test( 'innerHTML is a string', () => {
			...
			expect( parse( '<!-- wp:test --><!-- /wp:test -->' )[ 0 ] ).toHaveProperty(
				'innerHTML',
				expect.any( String )
			);
			expect( parse( '<!-- wp:test -->test<!-- /wp:test -->' )[ 0 ] ).toHaveProperty(
				'innerHTML',
				expect.any( String )
			);
		} );
	...
	describe( 'generic tests', () => {
		...
		test( 'treats void blocks and empty blocks identically', () => {
			...
			expect( parse( '<!-- wp:my/bus { "is": "fast" } /-->' ) ).toEqual(
				parse( '<!-- wp:my/bus { "is": "fast" } --><!-- /wp:my/bus -->' )
			);
		} );
	...
	describe( 'innerBlock placemarkers', () => {
		...
		test( 'block locations become null', () => {
			expect( parse( '<!-- wp:block --><!-- wp:void /--><!-- /wp:block -->' )[ 0 ] ).toHaveProperty(
				'innerContent',
				[ null ]
			);
		} );
		...
		test( 'HTML soup appears after blocks', () => {
			expect(
				parse( '<!-- wp:block --><!-- wp:void /-->After<!-- /wp:block -->' )[ 0 ]
			).toHaveProperty( 'innerContent', [ null, 'After' ] );
		} );

		test( 'HTML soup appears before blocks', () => {
			expect(
				parse( '<!-- wp:block -->Before<!-- wp:void /--><!-- /wp:block -->' )[ 0 ]
...
const hasPHP =
	'test' === process.env.NODE_ENV
		? ( () => {
				const process = require( 'child_process' ).spawnSync( 'php', [ '-r', 'echo 1;' ], {
					encoding: 'utf8',
				} );
...
export const phpTester = ( name, filename ) =>
	makeTest(
		...
		'test' === process.env.NODE_ENV
			? jsTester( ( doc ) => {
					const process = require( 'child_process' ).spawnSync( 'php', [ '-f', filename ], {
						input: doc,
						encoding: 'utf8',
